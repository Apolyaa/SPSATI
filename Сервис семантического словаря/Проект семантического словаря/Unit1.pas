...
procedure Initlist(x:byte); //очистка listbox1 и listbox2
var i:integer;
begin
With Form1 do begin
...
procedure FormZap; //формирование записи
var i:integer;
begin
...
Form1.label8.Caption:='№зап: '+inttostr(Mark+1);
Form1.label9.Caption:='Всего: '+inttostr(filesize(ftp));
end;
...
begin
ListBox1.Clear;
OpenDialog1.Title :='Открыть файл словаря'; OpenDialog1.FileName :='*.txt';
...
Assignfile(Ft,Fname); //связывает файловую переменную с именем файла
 ...
 v:=0;
 While not eof(Ft) do
   begin readln(Ft,S);ListBox1.Items.Append(S); inc(v); end; //загружаем словарь
    label2.Caption:='Всего: '+inttostr(v);
   Closefile(Ft);
 L:=length(fname);
 i:=pos('.',fname);
 fnametp:=fname;
...
Assignfile(Ftp,Fnametp);    //открываем типизированный файл для редактирования
if filesearch(fnametp,'')<>'' then
begin
  reset(ftp); FtpOk:=true;
  Mark:=0; // открываем на первой записи
  seek(ftp,Mark);
  read(ftp,zap);

label8.Caption:='№зап: '+inttostr(Mark+1);
label9.Caption:='Всего: '+inttostr(filesize(ftp));
for i:=0 to 9 do
begin
 listbox2.Items.Insert(i,zap.ms[i+1]); //загружаем типизированный файл
...
if Sv<>'-----' then
 ...
 listbox2.Items.Insert(nx1,Sv); //вставляем выделенный файл в listbox2
 ListBox2.Items.Delete(nx1+1); //удаляем *
 ListBox1.Items.Strings[ListBox1.ItemIndex]:='-----'; //вставляем пропуски вместо слова
 if nx1<9 then begin inc(nx1); ListBox2.ItemIndex:=nx1; end;

end else showmessage('Нужно выделить слово!');
end;
end;
end;
...
St:=ListBox1.Items.Strings[ListBox1.ItemIndex];
 listbox1.Items.Insert(n+1,Sv); //вставляем переносимое слово в listbox1
 ListBox1.Items.Delete(n);//удаляем пропуски
  ListBox2.Items.Delete(n1); //удаляем переносимое слово
  ListBox2.Items.Append('*');//добавляем *
  if nx1>1 then dec(nx1);
     end else showmessage('Нужно выделить 2 слова!');
  end;
...
Sv:=ListBox1.Items.Strings[ListBox1.ItemIndex];
 ...
 listbox3.Items.Insert(nx2,Sv);//вставляем переносимое слово в listbox3
 ListBox3.Items.Delete(nx2+1); //удаляем слово из listbox3
 ListBox1.Items.Strings[ListBox1.ItemIndex]:='-----';//вставляем прочерк вместо слова
...
n1:=ListBox3.ItemIndex;
ListBox1.Items.Insert(n+1,Sv);//вставляем  переносимое слово в listbox1
 ListBox1.Items.Delete(n);//удаляем прочерк
  ListBox3.Items.Delete(n1);//удаляем слово
...
begin
Initlist(10);
OpenDialog2.Title :='Открыть файл семантического словаря';
...
end else showmessage('Нужно открыть файл !');
end;
...
//создание семантического словаря
procedure TForm1.Button8Click(Sender: TObject);
begin
  SaveDialog1.Title := 'Создать семантический словарь';
...
Mark:= filesize(ftp);
 ...
 Initlist(10);
 FormZap;
 zap.ms[1]:=inttostr(Mark+1)+' запись';
 write(ftp,zap);
 Indzap;
end;
...
//переход на форму редактирования слова из listbox2
...
begin
if button7.Caption='Найти' then Markp:=-1;
flag:=false;
...
begin
 ...
 if button7.Caption='Найти' then begin
...
Mark:=Markp;
Indzap;
button7.Caption:='Найти далее';
end;
end;

end else showmessage('Нужно открыть файл семантического словаря');
end;
//удаление слова
...
listbox3.Items.Insert(n,'*');
 ...
 //переход на форму редактирования слова из listbox3
